...
export default function ProjectCard({project, avatars}) {
    ...
    const deleteProject = () => {
        axios.delete(`http://${backHost}:${backPort}/api/projects/delete/${project.id}`)
            .then(res => { if (res.status === 204) navigate('/projects'); });
    };
        ...
        >
            ...
            <div className="text-info">
                ...
                <div className="date-added">
                    <FaCalendarAlt /> {project.dateAdded}
                </div>
            </div>
                    ...
                    {({close}) => (
                        <NewProjectForm
                            ...
                            onSubmit={async formData => {
                                await axios.patch(
                                    `http://${backHost}:${backPort}/api/projects/update/${project.id}`,
                                    formData,
                    ...
                    {({close}) => (
                        <>
                            ...
                            <div className="modal-actions">
                                ...
                                <Button onClickFunction={() => { deleteProject(); close(); }}>Да</Button>
                            </div>
                        </>
                    )}
                </ModalWindow>
